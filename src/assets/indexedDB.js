...
export default {
    ...
    foreach: function (objectStoreName) {
        if (db != null) {
            // 执行事务，从对象仓库（表）中获取所有数据
            var request = db.transaction([objectStoreName], 'readwrite').objectStore(objectStoreName).openCursor()
    ...
    update: function (objectStoreName, argument) {
        if (db != null) {
            // 执行事务，添加数据到对象仓库（表）
            var request = db.transaction([objectStoreName], 'readwrite').objectStore(objectStoreName).put(argument);
    ...
    remove: function (objectStoreName, index) {
        if (db != null) {
            ...
            // 执行事务，从对象仓库（表）中获取所有数据
            var request = db.transaction([objectStoreName], 'readwrite').objectStore(objectStoreName).delete(index)
    ...
    searchFromIndex: function (objectStoreName, index, data) {
        if (db != null) {
            ...
            // 执行事务，从对象仓库（表）中获取所有数据
            var request = db.transaction([objectStoreName], 'readonly').objectStore(objectStoreName).index(index).getAll(data)
